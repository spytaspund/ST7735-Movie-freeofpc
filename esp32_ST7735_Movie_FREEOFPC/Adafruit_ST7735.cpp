...
Adafruit_ST7735::Adafruit_ST7735(int8_t cs, int8_t rs, int8_t sid, int8_t sclk, int8_t rst)
  : WIDTH(ST7735_TFTWIDTH), HEIGHT(ST7735_TFTHEIGHT_18) {
  _width = WIDTH;
  _height = HEIGHT;
  rotation = 0;
  _cs = cs;
...
void Adafruit_ST7735::drawBitmap(int16_t x, int16_t y, const uint8_t bitmap[],
...
void Adafruit_ST7735::printString(int16_t x, int16_t y, String st,
                                  uint16_t color, uint16_t bg, int8_t size_x,
                                  int8_t size_y) {
  char str[st.length() + 1] = {};
  st.toCharArray(str, st.length() + 1*size_x);
  int locx = x - 6*size_x;
  int locy = y;
  for (int i = 0; i <= st.length(); i++) {
    ...
    drawChar(locx, locy, str[i], color, bg, size_x, size_y);
  }
}
void Adafruit_ST7735::tprint(String st, uint16_t color, uint16_t bg, int8_t size_x, int8_t size_y) {
  char str[st.length() + 1] = {};
  st.toCharArray(str, st.length() + 1*size_x);
  locx = locx - 6*size_x;
  for (int i = 0; i <= st.length(); i++) {
...
void Adafruit_ST7735::setCursor(int x, int y){
  locx = x;
  locy = y;
}

int Adafruit_ST7735::getX(void){
  return locx;
}

int Adafruit_ST7735::getY(void){
  return locy;
}